...
// ══════════ STORAGE ══════════
function load(k)    { try { const v = localStorage.getItem('bg_' + k); return v ? JSON.parse(v) : null; } catch { return null; } }
function persist(k,v) {
  localStorage.setItem('bg_' + k, JSON.stringify(v));
  // Broadcast to any open website tabs so they refresh instantly.
  try {
    if (!window.__bgChan) window.__bgChan = new BroadcastChannel('bista_group_sync');
    window.__bgChan.postMessage({ key: k, ts: Date.now() });
...
function handleLogoUpload(input) {
  ...
  r.onload = e => {
    ...
    setVal('a-logo-url', d);
    const lp = document.getElementById('a-logo-preview');
    if (lp) { lp.src = d; lp.style.display = 'block'; }
  };
  r.readAsDataURL(f);
}
...
// For production: replace base64 with real file upload to a server or cloud storage (e.g. Cloudinary, S3).
// Base64 images work fine for moderate use but large images may exceed localStorage limits (~5MB total).
function handleLogoUpload(input) {
  toBase64(input, d => {
    setVal('a-logo-url', d);
    const lp = document.getElementById('a-logo-preview');
...
function handleImgUpload(input, targetId) {
  toBase64(input, d => setVal(targetId, d));
}
function toBase64(input, cb) {
  const f = input.files[0]; if (!f) return;
  const r = new FileReader();
